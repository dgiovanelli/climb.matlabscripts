...
nans_infs_count = 0;
% if sum(size(nodePositionXY_GroundTh)  ~= size(NODE_POSITION_ID_XY))
%     error('size(nodePositionXY_GroundTh)  ~= size(NODE_POSITION_ID_XY)');
% end

if TREAT_AS_STATIC == 0
    if size(GRAPH_EDGES_M_FILT,1) <= size(GRAPH_EDGES_M_GT,1)
        amountOfTimeSamples = size(GRAPH_EDGES_M_FILT,1);
    else
        amountOfTimeSamples = size(GRAPH_EDGES_M_GT,1);
...
meanPositioningError = Inf(size(NODE_POSITION_ID_XY,3),1);
nodePositionXY_transform = zeros(size(NODE_POSITION_ID_XY));
nodePositionXY_transform(:,1,:) = NODE_POSITION_ID_XY(:,1,:);
if ENABLE_FREE_TRANSFORMATION
    A_store = zeros(3,2,size(NODE_POSITION_ID_XY,3));
else
    A_store = zeros(2,2,size(NODE_POSITION_ID_XY,3));
end
...
%A_store = zeros(1,1,size(NODE_POSITION_ID_XY,3));
% if xstop_index > size(nodePositionXY_GroundTh,3)
%     xstop_index = size(nodePositionXY_GroundTh,3);
% end

%% CALCULATING TRANSFORMATION MATRIX
error_calculation_startSample = 1; %usually this should be 1
if error_calculation_startSample ~= 1
    warning('Attention: error_calculation_startSample = %d',error_calculation_startSample);
end
for timeIndexNo = error_calculation_startSample : amountOfTimeSamples
    if PLOT_VERBOSITY > 0
        %opts = optimset('Algorithm','lm-line-search');
        if ENABLE_FREE_TRANSFORMATION
            positiongErrorCost_an = @(A)positiongErrorCost_FreeTrans( A,nodePositionXY_GroundTh(:,:,timeIndexNo), NODE_POSITION_ID_XY(:,:,timeIndexNo) );
        else
            positiongErrorCost_an = @(A)positiongErrorCost_RotRefTrasl( A,nodePositionXY_GroundTh(:,:,timeIndexNo), NODE_POSITION_ID_XY(:,:,timeIndexNo) );
        end
        ...
        for nodeNo = 1:size(NODE_POSITION_ID_XY,1)
            %NODE_POSITION_ID_XY(nodeNo,2:3,timeNo) = ((A(1:2,1:2)*NODE_POSITION_ID_XY(nodeNo,2:3,timeNo)')+ A(3,:)')';
            %NODE_POSITION_ID_XY(nodeNo,2:3,timeNo) = ([-cos(A) , sin(A); sin(A),cos(A)]*NODE_POSITION_ID_XY(nodeNo,2:3,timeNo)')';
            if ENABLE_FREE_TRANSFORMATION
                nodePositionXY_transform(nodeNo,2:3,timeIndexNo) = ((A(1:2,1:2)*NODE_POSITION_ID_XY(nodeNo,2:3,timeIndexNo)')+ A(3,:)')';
            else %only rotation/reflection/translation are allowed
                if A(1,2) > 0
                    nodePositionXY_transform(nodeNo,2:3,timeIndexNo) = ([cos(2*A(1,1)) , sin(2*A(1,1)); sin(2*A(1,1)),-cos(2*A(1,1))]*NODE_POSITION_ID_XY(nodeNo,2:3,timeIndexNo)')- A(2,:)';
                else
                    nodePositionXY_transform(nodeNo,2:3,timeIndexNo) = ([cos(A(1,1)) , -sin(A(1,1)); sin(A(1,1)),cos(A(1,1))]*NODE_POSITION_ID_XY(nodeNo,2:3,timeIndexNo)')- A(2,:)';
                end
            end
    ...
    for nodeNo_m_GT = 1:size(nodePositionXY_GroundTh(:,:,1),1)-1
        nodeNo_m_LAY = find(NODE_POSITION_ID_XY(:,1,timeIndexNo) == nodePositionXY_GroundTh(nodeNo_m_GT,1,timeIndexNo));
        if length(nodeNo_m_LAY) == 1
            for nodeNo_i_GT = nodeNo_m_GT+1:size(nodePositionXY_GroundTh(:,:,1),1)
                nodeNo_i_LAY = find(NODE_POSITION_ID_XY(:,1,timeIndexNo) == nodePositionXY_GroundTh(nodeNo_i_GT,1,timeIndexNo));
                ...
                if length(nodeNo_i_LAY) == 1
                    % calculate link length on ground truth data (they are also stored in GRAPH_EDGES_M_GT, but links_GroundTh has a different order wrt LINKS)
                    ...
                    link_length_mi_LAY = sqrt( (NODE_POSITION_ID_XY(nodeNo_m_LAY,2,timeIndexNo)-NODE_POSITION_ID_XY(nodeNo_i_LAY,2,timeIndexNo))^2 + (NODE_POSITION_ID_XY(nodeNo_m_LAY,3,timeIndexNo)-NODE_POSITION_ID_XY(nodeNo_i_LAY,3,timeIndexNo))^2 );
                    ...
                    if length(linkNo_ACQ) == 1
                        link_length_mi_ACQ = GRAPH_EDGES_M_FILT(timeIndexNo,linkNo_ACQ);
                        ...
                        warning('The same link has been found twice in GRAPH_EDGES_M_FILT or it is not present');
                    end
                    ...
                    warning('A node is missing from NODE_POSITION_ID_XY or is repeated more than once');
                end
            end
        else
            warning('A node is missing from NODE_POSITION_ID_XY or is repeated more than once');
...
fprintf('Nans or Infs count: %d, or %.2f %% of total LINKS samples.\n\n',nans_infs_count,100*nans_infs_count/(size(GRAPH_EDGES_M,2)*amountOfTimeSamples));
