...
if TREAT_AS_STATIC == 0
    ...
    IDX_pre_localization = -Inf*ones(size(AVAILABLE_IDs,1),2,1); %the first column will containt the node ID, the second will contain the cluster number
    IDX_post_localization = -Inf*ones(size(AVAILABLE_IDs,1),2,1); %the first column will containt the node ID, the second will contain the cluster number
