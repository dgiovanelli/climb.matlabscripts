%ERRORS PLOT
if PLOT_VERBOSITY > 2
    t_plot = (start_selected_cut_index+(1:1:size(link_length_mi_error,3)))*(winc_sec*DECIMATION_AFTER_FILT_FACTOR);
    figure(401);
    plot(t_plot, permute(max(link_length_mi_error(3,:,:)),[3,2,1]), 'b.', t_plot, permute(mean(link_length_mi_error(3,:,:)),[3,2,1]), 'g.', t_plot, permute(min(link_length_mi_error(3,:,:)),[3,2,1]), 'r.');
    grid on;
    ...
    figure(402);
    plot(t_plot, permute(max(link_length_mi_error(4,:,:)),[3,2,1]), 'b.', t_plot, permute(mean(link_length_mi_error(4,:,:)),[3,2,1]), 'g.', t_plot, permute(min(link_length_mi_error(4,:,:)),[3,2,1]), 'r.');
    grid on;
    ...
    figure(403);
    plot(t_plot, permute(max(link_length_mi_error(5,:,:)),[3,2,1]), 'b.', t_plot, permute(mean(link_length_mi_error(5,:,:)),[3,2,1]), 'g.', t_plot, permute(min(link_length_mi_error(5,:,:)),[3,2,1]), 'r.');
    grid on;
    ...
    figure(404);
    plot(t_plot, permute(max(link_length_mi_error(6,:,:)),[3,2,1]), 'b.', t_plot, permute(mean(link_length_mi_error(6,:,:)),[3,2,1]), 'g.', t_plot, permute(min(link_length_mi_error(6,:,:)),[3,2,1]), 'r.');
    grid on;
...
%%ERROR SNRQ
rmse_after_loc = zeros(1,size(link_length_mi_error,3));
rmse_after_loc_percent = zeros(1,size(link_length_mi_error,4));
for timeNo = 1:size(link_length_mi_error,3)
    rmse_after_loc(timeNo) = sum(link_length_mi_error(3,:,timeNo).^2)/size(link_length_mi_error,2);
    rmse_after_loc_percent(timeNo) = sum(link_length_mi_error(4,:,timeNo).^2)/size(link_length_mi_error,2);
end
...
%%ERROR SNRQ
rmse_before_loc = zeros(1,size(link_length_mi_error,5));
rmse_before_loc_percent = zeros(1,size(link_length_mi_error,6));
for timeNo = 1:size(link_length_mi_error,3)
    rmse_before_loc(timeNo) = sum(link_length_mi_error(5,:,timeNo).^2)/size(link_length_mi_error,2);
    rmse_before_loc_percent(timeNo) = sum(link_length_mi_error(6,:,timeNo).^2)/size(link_length_mi_error,2);
end
